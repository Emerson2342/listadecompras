...
export default function HigienePessoal() {
  ...
  useEffect(() => {
    const listaHigienePessoal = listaGeral
      .filter((item) => item.tipo === "HigienePessoal")
      .sort((a, b) => a.produto.localeCompare(b.produto));
    setHigienePessoal(listaHigienePessoal);
  }, [listaGeral]);
      ...
      : setListaGeral((listaAntiga) => {
        return listaAntiga.map((item) => {
          if (item.id === id) {
            ...
            return { ...item, cart: !item.cart, selected: false };
          }
          return item;
        });
      });
  ...
  const renderItem = ({ item, index }) => (
    <View style={styles.higienePessoalContainer}>
    ...
    <View>
      ...
      <View>
        ...
        <TouchableOpacity
          ...
          onLongPress={() => alert(JSON.stringify(higienePessoal, null, 2))}
        >
